...
function Surf3D({ S, r, sigma, type, q, what, strike, T: uT, style }) {
  ...
  useEffect(() => {
    ...
    const cs = {
      price: [[0, "#eee8d5"], [0.2, "#d5cfbd"], [0.5, "#2aa198"], [0.8, "#dc322f"], [1, "#b58900"]],
      delta: [[0, "#eee8d5"], [0.5, "#6c71c4"], [1, "#b58900"]], gamma: [[0, "#eee8d5"], [0.5, "#268bd2"], [1, "#b58900"]],
      theta: [[0, "#b58900"], [0.5, "#dc322f"], [1, "#eee8d5"]], vega: [[0, "#eee8d5"], [0.5, "#268bd2"], [1, "#2aa198"]],
      rho: [[0, "#eee8d5"], [0.5, "#6c71c4"], [1, "#dc322f"]], earlyPremium: [[0, "#eee8d5"], [0.5, "#2aa198"], [1, "#b58900"]]
    };
    const mk = [];
    if (strike > 0 && uT > 0) {
      ...
      mk.push({
        ...
        marker: { size: 6, color: "#b58900", symbol: "diamond", line: { color: "#073642", width: 1 } }, showlegend: false,
    ...
    const ax = { gridcolor: "rgba(181,137,0,0.12)", color: "#657b83", backgroundcolor: "rgba(0,0,0,0)", showbackground: false };
    const data = [{
      type: "surface", x: strikes, y: times, z, colorscale: cs[what] || cs.price, showscale: false,
      contours: { z: { show: true, usecolormap: true, highlightcolor: "rgba(181,137,0,0.25)" } },
    ...
    const layout = {
      ...
      hoverlabel: { bgcolor: "#fdf6e3", bordercolor: "#b58900", font: { color: "#073642", family: "DM Mono", size: 11 } }
    };
  ...
  useEffect(() => {
    ...
    const traces = paths.map((p, idx) => {
      ...
      return {
        ...
        line: { color: itm ? "rgba(42,161,152,0.30)" : "rgba(220,50,47,0.15)", width: 0.8 },
        showlegend: false, hoverinfo: "skip"
      };
    });
    traces.push({
      x: [timeAxis[0], timeAxis[timeAxis.length - 1]], y: [K, K], type: "scatter", mode: "lines",
      line: { color: "#b58900", width: 2, dash: "dash" }, name: `Strike R$${K.toFixed(2)}`, showlegend: true
    });
    traces.push({
      x: [timeAxis[0], timeAxis[timeAxis.length - 1]], y: [S, S], type: "scatter", mode: "lines",
      line: { color: "#268bd2", width: 1.5, dash: "dot" }, name: `Spot R$${S.toFixed(2)}`, showlegend: true
    });
    ...
    const layout = {
      ...
      font: { family: "DM Mono", color: "#657b83", size: 10 },
      xaxis: { title: "Tempo (anos)", gridcolor: "#d5cfbd", color: "#839496", zerolinecolor: "#d5cfbd" },
      yaxis: { title: "Preço do Ativo (R$)", gridcolor: "#d5cfbd", color: "#839496", zerolinecolor: "#d5cfbd" },
      margin: { l: 50, r: 20, t: 30, b: 40 },
      legend: { bgcolor: "rgba(0,0,0,0)", font: { color: "#657b83", size: 10 }, x: 0.02, y: 0.98 },
      annotations: [{
        ...
        showarrow: false, font: { color: "#b58900", size: 11, family: "DM Mono" }, align: "right", bgcolor: "rgba(253,246,227,0.85)", borderpad: 6
      }],
      hoverlabel: { bgcolor: "#fdf6e3", bordercolor: "#b58900", font: { color: "#073642", family: "DM Mono" } }
    };
...
function MathPanel({ S, K, T, r, sig, type, q, style, mkt, result }) {
  ...
  return (
    <div className="no-scroll" style={{
      background: "#f5eed6", border: "1px solid #d5cfbd", borderRadius: 8, padding: 16, fontSize: 11,
      fontFamily: "'DM Mono',monospace", color: "#657b83", lineHeight: 2, overflowX: "auto"
    }}>
      <div style={{ color: "#b58900", fontSize: 12, marginBottom: 8, fontWeight: 600 }}>
        RACIOCÍNIO MATEMÁTICO — {isAm ? "BINOMIAL CRR" : "BLACK-SCHOLES-MERTON"}
      </div>
      <div style={{ color: "#93a1a1", marginBottom: 12 }}>═══════════════════════════════════════</div>

      <div style={{ color: "#586e75" }}>1. INPUTS UTILIZADOS</div>
      <div style={{ paddingLeft: 12, color: "#657b83" }}>
      ...
      {!isAm && (
        <>
          <div style={{ color: "#93a1a1", margin: "12px 0" }}>───────────────────────────</div>
          <div style={{ color: "#586e75" }}>2. CÁLCULO d₁ e d₂</div>
          <div style={{ paddingLeft: 12, color: "#657b83" }}>
          ...
          <div style={{ color: "#93a1a1", margin: "12px 0" }}>───────────────────────────</div>
          <div style={{ color: "#586e75" }}>3. N(d₁) e N(d₂) — CDF Normal Padrão</div>
          <div style={{ paddingLeft: 12, color: "#657b83" }}>
          ...
          <div style={{ color: "#93a1a1", margin: "12px 0" }}>───────────────────────────</div>
          <div style={{ color: "#586e75" }}>4. PREÇO BSM ({type.toUpperCase()})</div>
          <div style={{ paddingLeft: 12, color: "#657b83" }}>
            {type === "call" ? <>
            ...
            <span style={{ color: "#b58900", fontSize: 13, fontWeight: 700 }}>Fair Value BSM = R${result.price1.toFixed(4)}</span>
          </div>
        </>
      )}
      ...
      {isAm && (
        <>
          <div style={{ color: "#93a1a1", margin: "12px 0" }}>───────────────────────────</div>
          <div style={{ color: "#586e75" }}>2. ÁRVORE BINOMIAL CRR (400 steps)</div>
          <div style={{ paddingLeft: 12, color: "#657b83" }}>
            ...
            <span style={{ color: "#b58900", fontSize: 13, fontWeight: 700 }}>Fair Value Binomial = R${result.price1.toFixed(4)}</span><br />
      ...
      <div style={{ color: "#93a1a1", margin: "12px 0" }}>───────────────────────────</div>
      <div style={{ color: "#586e75" }}>{isAm ? "3" : "5"}. VOLATILIDADE IMPLÍCITA (via Bisection)</div>
      <div style={{ paddingLeft: 12, color: "#657b83" }}>
        Busca σ_iv tal que Modelo(σ_iv) = Preço_Mercado = R${mkt.toFixed(4)}<br />
        Intervalo: [0.1%, 800%] · 300 iterações · tol = 1e-9<br />
        <span style={{ color: iv > sig ? "#dc322f" : "#2aa198", fontWeight: 600 }}>σ_iv = {(iv * 100).toFixed(4)}%</span>
        {iv > sig ? " (mercado precifica MAIS risco que histórico)" : " (mercado precifica MENOS risco)"}
      </div>

      <div style={{ color: "#93a1a1", margin: "12px 0" }}>───────────────────────────</div>
      <div style={{ color: "#586e75" }}>{isAm ? "4" : "6"}. VEREDITO</div>
      <div style={{ paddingLeft: 12, color: "#657b83" }}>
        ...
        <span style={{ color: result.verdict1 === "CARA" ? "#dc322f" : result.verdict1 === "BARATA" ? "#2aa198" : "#657b83", fontSize: 13, fontWeight: 700 }}>
...
const Pill = ({ children, active, onClick }) => <button onClick={onClick} style={{
  ...
  fontWeight: 500, background: active ? "rgba(181,137,0,0.14)" : "transparent", color: active ? "#b58900" : "#93a1a1"
}}>{children}</button>;

const Met = ({ label, value, unit, color, small }) => <div style={{ padding: small ? "8px 10px" : "12px 14px", background: "#f5eed6", borderRadius: 8, borderLeft: `2px solid ${color || "#d5cfbd"}` }}>
  <div style={{ fontSize: 10, color: "#839496", letterSpacing: 1.2, marginBottom: 4, fontFamily: "'DM Mono',monospace" }}>{label}</div>
  <div style={{ display: "flex", alignItems: "baseline", gap: 3 }}>
    <span style={{ fontSize: small ? 14 : 18, fontWeight: 600, color: color || "#586e75", fontFamily: "'DM Mono',monospace" }}>{value}</span>
    {unit && <span style={{ fontSize: 10, color: "#839496" }}>{unit}</span>}
  </div>
</div>;

const Sep = () => <div style={{ height: 1, background: "linear-gradient(90deg,transparent,#d5cfbd,transparent)", margin: "12px 0" }} />;
...
export default function App() {
  ...
  const vc = v => v === "CARA" ? "#dc322f" : v === "BARATA" ? "#2aa198" : "#657b83";
  ...
  return (
    <div style={ST.root}>
      ...
      *{box-sizing:border-box;margin:0;padding:0}
      ...
      ::-webkit-scrollbar-thumb{background:#ccc9b6;border-radius:2px}
      ...
      .resizer-handle { width: 2px; height: 30px; background: #ccc9b6; border-radius: 2px; transition: background 0.2s; }
      .resizer:hover .resizer-handle, .resizer.active .resizer-handle { background: #b58900; }

      body{overflow:hidden;background:#fdf6e3}
      input[type=number]{-moz-appearance:textfield}
      input::-webkit-outer-spin-button,input::-webkit-inner-spin-button{-webkit-appearance:none}
      .fi{display:flex;flex-direction:column;gap:4px}
      .fi label{font-size:10px;color:#839496;letter-spacing:1px;font-family:'DM Mono',monospace;text-transform:uppercase}
      .fi input,.fi select{background:#f5eed6;border:1px solid #d5cfbd;color:#073642;padding:8px 10px;border-radius:5px;font-family:'DM Mono',monospace;font-size:12px;outline:none;transition:border-color 0.2s}
      .fi input:focus,.fi select:focus{border-color:#b58900}
      .fi select option{background:#f5eed6;color:#073642}
      @keyframes fadeUp{from{opacity:0;transform:translateY(6px)}to{opacity:1;transform:translateY(0)}}
      @keyframes blink{0%,100%{opacity:1}50%{opacity:0.3}}
      .tu{animation:flashG 0.8s ease-out}.td{animation:flashR 0.8s ease-out}
      @keyframes flashG{0%{background:rgba(42,161,152,0.18)}100%{background:transparent}}
      @keyframes flashR{0%{background:rgba(220,50,47,0.18)}100%{background:transparent}}
    ...
    `}</style>
      ...
      <header style={ST.hd}>
        <div style={{ display: "flex", alignItems: "center", gap: 16 }}>
          <div style={ST.logo}><span style={{ fontSize: 11, fontWeight: 700, color: "#b58900", lineHeight: 1, textAlign: "center" }}>vinicin<br />goat</span></div>
          <div style={{ display: "flex", gap: 3, background: "#e4deca", padding: 3, borderRadius: 6 }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 14 }}>
          <div style={{ padding: "4px 10px", borderRadius: 4, fontSize: 10, fontWeight: 600, fontFamily: "'DM Mono',monospace", background: "rgba(181,137,0,0.10)", color: "#b58900", border: "1px solid rgba(181,137,0,0.20)" }}>ENGINE v3.5 JS</div>
          <div style={{ display: "flex", alignItems: "center", gap: 6, fontSize: 11, color: "#2aa198", fontFamily: "'DM Mono',monospace" }}>
            <div style={{ width: 5, height: 5, borderRadius: "50%", background: "#2aa198", animation: "blink 2.5s infinite" }} />
      ...
      {/* ════════ SCREENER ════════ */}
      {tab === "screener" && (
        <div style={{ display: "flex", flexDirection: "column", height: "calc(100vh - 72px)", background: "#eee8d5", borderRadius: 10, border: "1px solid #d5cfbd" }}>
          <div style={{ padding: "12px 16px", borderBottom: "1px solid #d5cfbd", display: "flex", gap: 14, alignItems: "center", flexWrap: "wrap" }}>
            <div style={{ fontSize: 12, fontWeight: 600, color: "#586e75" }}>ATIVO:</div>
            <input type="text" list="tk" placeholder="PETR4" value={asset} onChange={e => setAsset(e.target.value.toUpperCase())}
              style={{ background: "#f5eed6", border: "1px solid #ccc9b6", color: "#b58900", padding: "7px 12px", borderRadius: 5, fontFamily: "'DM Mono',monospace", fontSize: 14, fontWeight: 700, width: 130, outline: "none" }} />

            <div style={{ display: "flex", alignItems: "center", gap: 8, background: "#f5eed6", padding: "6px 14px", borderRadius: 6, border: "1px solid #d5cfbd" }}>
              <span style={{ fontSize: 10, color: "#839496", fontFamily: "'DM Mono',monospace" }}>SPOT</span>
              <span style={{ fontSize: 16, fontWeight: 700, color: "#073642", fontFamily: "'DM Mono',monospace" }}>R${fmtR(currentSpot)}</span>
            </div>

            <select value={expF} onChange={e => setExpF(e.target.value)} style={{ background: "#f5eed6", border: "1px solid #ccc9b6", color: "#586e75", padding: "7px 12px", borderRadius: 5, fontFamily: "'DM Mono',monospace", fontSize: 12, outline: "none" }}>
              <option value="ALL">Todos Vencimentos</option>
              {EXPIRIES.map(e => <option key={e} value={e}>{e}</option>)}
            </select>

            <div style={{ display: "flex", gap: 2, background: "#f5eed6", padding: 3, borderRadius: 5 }}>
              {[["ALL", "TODOS"], ["call", "CALL"], ["put", "PUT"]].map(([k, l]) =>
                <button key={k} onClick={() => setTypeF(k)} style={{
                  ...
                  background: typeF === k ? (k === "call" ? "rgba(38,139,210,0.15)" : k === "put" ? "rgba(220,50,47,0.15)" : "rgba(181,137,0,0.12)") : "transparent",
                  color: typeF === k ? (k === "call" ? "#268bd2" : k === "put" ? "#dc322f" : "#b58900") : "#93a1a1"
                }}>{l}</button>)}
            </div>

            <div style={{ flex: 1 }} />
            <div style={{ fontSize: 11, color: "#839496", fontFamily: "'DM Mono',monospace" }}>Att: {tick.toLocaleTimeString()}</div>
          </div>
          ...
          <div className="no-scroll" style={{ flex: 1 }}>
            <table style={{ width: "100%", borderCollapse: "collapse", textAlign: "right", fontFamily: "'DM Mono',monospace", fontSize: 12 }}>
              <thead style={{ position: "sticky", top: 0, background: "#e4deca", zIndex: 2 }}>
                <tr style={{ color: "#657b83", borderBottom: "1px solid #ccc9b6" }}>
              ...
              <tbody>
                {filteredChain.map(o => {
                  ...
                  return (<tr key={o.ticker + o.expiry} className={tc} style={{ borderBottom: "1px solid #d5cfbd" }}>
                    <td style={{ ...ST.td, color: "#b58900", fontWeight: 600, textAlign: "left" }}>{o.ticker}</td>
                    <td style={{ ...ST.td, color: "#657b83", textAlign: "left" }}>{o.underlying}</td>
                    <td style={{ ...ST.td, color: "#073642", fontWeight: 600 }}>{fmtR(o.spot)}</td>
                    <td style={{ ...ST.td, color: o.type === "call" ? "#268bd2" : "#dc322f", fontWeight: 600 }}>{o.type.toUpperCase()}</td>
                    <td style={ST.td}>{fmtR(o.strike)}</td>
                    <td style={{ ...ST.td, color: "#839496", fontSize: 10 }}>{o.expiry}</td>
                    <td style={{ ...ST.td, color: "#2aa198", fontWeight: 600 }}>{fmtR(o.bid)}</td>
                    <td style={{ ...ST.td, color: "#dc322f", fontWeight: 600 }}>{fmtR(o.ask)}</td>
                    <td style={{ ...ST.td, color: "#268bd2", fontWeight: 600 }}>{fmtR(o.fairValue)}</td>
                    <td style={{ ...ST.td, color: o.delta > 0 ? "#b58900" : "#dc322f" }}>{o.delta >= 0 ? "+" : ""}{o.delta.toFixed(3)}</td>
                    <td style={ST.td}><span style={{
                      padding: "4px 8px", borderRadius: 4, fontSize: 10, fontWeight: 600,
                      background: vrd === "CARA" ? "rgba(220,50,47,0.10)" : vrd === "BARATA" ? "rgba(42,161,152,0.10)" : "rgba(0,0,0,0.04)",
                      color: vrd === "CARA" ? "#dc322f" : vrd === "BARATA" ? "#2aa198" : "#657b83"
      ...
      {tab === "mc" && (
        <div style={{ display: "flex", height: "calc(100vh - 72px)" }}>
          ...
          <div className="no-scroll" style={{ ...ST.pn, width: leftW, flexShrink: 0 }}>
            <div style={ST.sh}><span style={{ color: "#b58900" }}>MC</span> PARÂMETROS</div>
            ...
            <Sep />
            <div style={{ fontSize: 10, color: "#839496", fontFamily: "'DM Mono',monospace", lineHeight: 1.8 }}>
              <span style={{ color: "#2aa198" }}>Verde</span> = paths ITM no vencimento<br />
              <span style={{ color: "#dc322f" }}>Vermelho</span> = paths OTM<br />
              Linha <span style={{ color: "#b58900" }}>dourada tracejada</span> = Strike<br />
              Linha <span style={{ color: "#268bd2" }}>azul pontilhada</span> = Spot atual<br /><br />
              <span style={{ color: "#586e75" }}>150 caminhos GBM simulados com antithetic variates. Cada recarga gera novos paths no navegador.</span>
            </div>
          </div>
          ...
          <div style={{ ...ST.pn, flex: 1, minWidth: 0, padding: 0, display: "flex", flexDirection: "column", overflow: "hidden" }}>
            <div style={{ padding: "12px 16px", borderBottom: "1px solid #d5cfbd", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
              <div style={ST.sh}><span style={{ color: "#b58900" }}>◇</span> SIMULAÇÃO MONTE CARLO — {optT.toUpperCase()} K={strike} T={T.toFixed(3)}a</div>
              <button onClick={() => setEnable3D(!enable3D)} style={{
                padding: "5px 12px", fontSize: 10, border: "1px solid " + (enable3D ? "#dc322f" : "#2aa198"), borderRadius: 4, cursor: "pointer",
                fontFamily: "'DM Mono',monospace", fontWeight: 600,
                background: enable3D ? "rgba(220,50,47,0.10)" : "rgba(42,161,152,0.10)",
                color: enable3D ? "#dc322f" : "#2aa198"
              }}>{enable3D ? "DESLIGAR" : "LIGAR GRÁFICO"}</button>
            </div>
            <div style={{ flex: 1, minHeight: 0, background: "radial-gradient(ellipse at 50% 80%,#f0e9d5,#fdf6e3)" }}>
              {enable3D ? <MCViz S={spot} K={strike} T={T} r={r} sigma={sig} type={optT} q={q} /> : (
                <div style={{ display: "flex", alignItems: "center", justifyContent: "center", height: "100%", color: "#ccc9b6", fontFamily: "'DM Mono',monospace", fontSize: 12 }}>
      ...
      {tab === "single" && (
        <div style={{ display: "flex", height: "calc(100vh - 72px)" }}>
          ...
          {/* LEFT INPUTS */}
          <div className="no-scroll" style={{ ...ST.pn, width: leftW, flexShrink: 0 }}>
            <div style={ST.sh}><span style={{ color: "#b58900" }}>01</span> PARÂMETROS</div>
            <div style={{ display: "flex", background: "#f5eed6", borderRadius: 5, padding: 3, border: "1px solid #d5cfbd", gap: 2, marginBottom: 10 }}>
              {[["europeia", "EUROPEIA", "EU"], ["americana", "AMERICANA", "US"]].map(([k, l, ic]) =>
                <button key={k} onClick={() => setStyle(k)} style={{
                  ...
                  background: style === k ? (k === "europeia" ? "rgba(38,139,210,0.12)" : "rgba(220,50,47,0.12)") : "transparent",
                  color: style === k ? (k === "europeia" ? "#268bd2" : "#dc322f") : "#93a1a1"
            ...
            <div className="fi" style={{ marginTop: 8 }}><label>Vencimento</label><input type="date" value={expiry} onChange={e => setExpiry(e.target.value)} /></div>
            <div style={{ display: "flex", gap: 6, marginTop: 8, fontSize: 10, fontFamily: "'DM Mono',monospace", color: "#93a1a1", flexWrap: "wrap" }}>
              <span style={{ background: "#f5eed6", padding: "3px 6px", borderRadius: 3 }}>{days}d</span>
              <span style={{ background: "#f5eed6", padding: "3px 6px", borderRadius: 3 }}>T={T.toFixed(4)}</span>
              <span style={{
                background: mLabel === "ITM" ? "rgba(42,161,152,0.10)" : mLabel === "OTM" ? "rgba(220,50,47,0.10)" : "#f5eed6",
                color: mLabel === "ITM" ? "#2aa198" : mLabel === "OTM" ? "#dc322f" : "#839496", padding: "3px 6px", borderRadius: 3
              }}>{mLabel}</span>
              <span style={{
                background: isAm ? "rgba(220,50,47,0.10)" : "rgba(38,139,210,0.10)",
                color: isAm ? "#dc322f" : "#268bd2", padding: "3px 6px", borderRadius: 3
              }}>{isAm ? "AMER" : "EUR"}</span>
            </div>
            <Sep />
          ...
          <div style={{ ...ST.pn, flex: 1, minWidth: 0, padding: 0, display: "flex", flexDirection: "column", overflow: "hidden" }}>
            <div style={{ padding: "12px 16px 6px", display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: 6 }}>
              <div style={{ display: "flex", gap: 3, background: "#f5eed6", padding: 3, borderRadius: 5 }}>
              ...
              <div style={{ display: "flex", gap: 6, alignItems: "center" }}>
                {vizTab === "surface" && (
                  <div style={{ display: "flex", gap: 2, background: "#f5eed6", padding: 3, borderRadius: 5, flexWrap: "wrap" }}>
            ...
            <div style={{ flex: 1, minHeight: 0, background: "radial-gradient(ellipse at 50% 80%,#f0e9d5,#fdf6e3)" }}>
              ...
              {!enable3D && (
                <div style={{ display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", height: "100%", gap: 16, padding: 32 }}>
                  <svg width="48" height="48" viewBox="0 0 24 24" fill="none" stroke="#ccc9b6" strokeWidth="1.5">
                    <path d="M12 3L1 9l11 6 11-6-11-6z" /><path d="M1 9v8l11 6 11-6V9" /><path d="M12 15v8" />
                  </svg>
                  <div style={{ fontSize: 14, color: "#93a1a1", fontFamily: "'DM Mono',monospace", textAlign: "center" }}>
                    Visualização 3D desativada para melhor performance
                  </div>
                  {comp && vizTab === "surface" && (
                    <div style={{ marginTop: 12, padding: 16, background: "#f5eed6", borderRadius: 8, border: "1px solid #d5cfbd", width: "100%", maxWidth: 500 }}>
                      <div style={{ fontSize: 10, color: "#839496", fontFamily: "'DM Mono',monospace", letterSpacing: 1.5, marginBottom: 10 }}>RESUMO RÁPIDO (sem 3D)</div>
                      ...
                      <div style={{ display: "grid", gridTemplateColumns: "repeat(5,1fr)", gap: 6, marginTop: 8 }}>
                        <Met label="Δ" value={comp.greeks.delta.toFixed(3)} color="#b58900" small />
                        <Met label="Γ" value={comp.greeks.gamma.toFixed(4)} color="#268bd2" small />
                        <Met label="Θ" value={comp.greeks.theta.toFixed(4)} color="#dc322f" small />
                        <Met label="ν" value={comp.greeks.vega.toFixed(4)} color="#2aa198" small />
                        <Met label="ρ" value={comp.greeks.rho.toFixed(4)} color="#6c71c4" small />
                      </div>
                    </div>
                  )}
                </div>
            ...
            <div style={{ padding: "4px 16px 8px", display: "flex", justifyContent: "space-between", fontSize: 10, fontFamily: "'DM Mono',monospace", color: "#93a1a1" }}>
          ...
          <div className="no-scroll" style={{ ...ST.pn, width: rightW, flexShrink: 0 }}>
            <div style={ST.sh}><span style={{ color: "#b58900" }}>03</span> RESULTADO</div>
            {comp && (<div style={{ animation: "fadeUp 0.3s ease" }}>
              {/* Verdict */}
              <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "12px 0", borderBottom: "1px solid #d5cfbd" }}>
                <div>
                  <div style={{ fontSize: 9, color: "#93a1a1", letterSpacing: 1.5, fontFamily: "'DM Mono',monospace", marginBottom: 3 }}>VEREDITO {comp.label1}</div>
                  <div style={{ fontSize: 24, fontWeight: 700, color: vc(comp.verdict1), fontFamily: "'Instrument Sans',sans-serif" }}>{comp.verdict1}</div>
                </div>
                <div style={{ textAlign: "right" }}>
                  <div style={{ fontSize: 9, color: "#93a1a1", letterSpacing: 1, fontFamily: "'DM Mono',monospace", marginBottom: 3 }}>SPREAD</div>
              ...
              {comp.mcSe > 0 && <div style={{ fontSize: 9, color: "#93a1a1", marginTop: 3, fontFamily: "'DM Mono',monospace", textAlign: "right" }}>MC±{comp.mcSe.toFixed(4)}</div>}

              {isAm && comp.earlyExPremium > 0.0001 && (<>
                <Sep />
                <div style={{ padding: "10px 12px", borderRadius: 6, background: "rgba(181,137,0,0.06)", border: "1px solid rgba(181,137,0,0.16)" }}>
                  <div style={{ fontSize: 9, color: "#839496", letterSpacing: 1, fontFamily: "'DM Mono',monospace", marginBottom: 4 }}>PRÊMIO EXERCÍCIO ANTECIPADO</div>
                  <span style={{ fontSize: 18, fontWeight: 700, fontFamily: "'DM Mono',monospace", color: "#b58900" }}>R${comp.earlyExPremium.toFixed(4)}</span>
                  <span style={{ fontSize: 10, color: "#839496", marginLeft: 8 }}>({(comp.earlyExPremium / comp.price1 * 100).toFixed(2)}%)</span>
                </div>
              </>)}

              <Sep />
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 6 }}>
                <Met label="VOL HIST" value={`${vol.toFixed(1)}`} unit="%" color="#b58900" small />
                <Met label="VOL IMPL" value={`${(comp.iv * 100).toFixed(1)}`} unit="%" color={comp.iv > sig ? "#dc322f" : "#2aa198"} small />
              </div>

              <Sep />
              <div style={{ fontSize: 9, color: "#839496", letterSpacing: 1.5, fontFamily: "'DM Mono',monospace", marginBottom: 6 }}>GREGAS {isAm ? "(FD)" : "(ANALÍTICAS)"}</div>
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 5 }}>
                {[{ l: "Delta", v: comp.greeks.delta, c: "#b58900", f: 4 }, { l: "Gamma", v: comp.greeks.gamma, c: "#268bd2", f: 5 },
                { l: "Theta", v: comp.greeks.theta, c: "#dc322f", f: 5, u: "/du" }, { l: "Vega", v: comp.greeks.vega, c: "#2aa198", f: 5, u: "/1%" },
                { l: "Rho", v: comp.greeks.rho, c: "#6c71c4", f: 5, u: "/1%" }
              ...
              <button onClick={() => setShowMath(!showMath)} style={{
                width: "100%", padding: "10px 0", border: "1px solid #d5cfbd", borderRadius: 5,
                background: showMath ? "rgba(181,137,0,0.06)" : "transparent", color: showMath ? "#b58900" : "#839496",
              ...
              <div style={{ marginTop: 10, fontSize: 9, color: "#93a1a1", fontFamily: "'DM Mono',monospace", lineHeight: 1.8 }}>
...
const ST = {
  root: { background: "#fdf6e3", height: "100vh", width: "100vw", overflow: "hidden", padding: "0 16px", fontFamily: "'Instrument Sans',sans-serif", color: "#657b83" },
  hd: { display: "flex", justifyContent: "space-between", alignItems: "center", height: 56, borderBottom: "1px solid #d5cfbd", marginBottom: 12, paddingTop: 8 },
  logo: { width: 40, height: 40, borderRadius: 7, border: "1px solid #b58900", display: "flex", alignItems: "center", justifyContent: "center" },
  tabI: { background: "transparent", color: "#93a1a1", border: "none", padding: "6px 14px", borderRadius: 5, cursor: "pointer", fontSize: 12, fontWeight: 500 },
  tabA: { background: "#e4deca", color: "#073642", border: "none", padding: "6px 14px", borderRadius: 5, cursor: "pointer", fontSize: 12, fontWeight: 600 },
  pn: { background: "#eee8d5", borderRadius: 10, padding: 16, border: "1px solid #d5cfbd", overflowY: "auto", height: "100%" },
  sh: { display: "flex", alignItems: "center", gap: 8, marginBottom: 12, fontSize: 11, fontWeight: 600, color: "#839496", fontFamily: "'DM Mono',monospace", letterSpacing: 1.5 },
  btn: { width: "100%", padding: "12px 0", border: "1px solid #b58900", borderRadius: 5, background: "rgba(181,137,0,0.08)", color: "#b58900", fontFamily: "'DM Mono',monospace", fontSize: 11, fontWeight: 500, letterSpacing: 2, cursor: "pointer" },
